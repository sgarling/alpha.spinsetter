...
class Bot extends CI_Controller {
	...
	public function fixArtwork(){
		...
		$songs = $this->SongsModel->get(999999);
		$c=0;
		foreach($songs as $s){
			if(!file_exists('./artwork/'.$s->Artwork)){
				//echo $s->IdSong."<br>";
				//$this->getMetadata($s->UrlSong,$s->TypeSource);
				$c++;
			}
		}
		echo $c;
	...
	public function parseFeeds(){
		...
		foreach($blogs as $blog){		
			...
			$sources = $this->parseCandidates($blog->IdBlog,true);
			...
			foreach($sources['urlSongs'] as $k=>$urlSong){
				...
				$meta= $this->getMetadata($song['UrlSong'],$song['TypeSource']);
				if(!$meta){
					//No metadata
					continue;
				}
		...
		echo"Added ".count($songs)." new songs.";
	...
	public function parseCandidates($IdBlog,$return=false){
		$this->load->model('BlogsModel');
		$this->load->model('SongsModel');
